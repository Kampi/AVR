...
static char __Representation[]= "0123456789ABCDEF";
	...
	*/
	...
	bool __USART_Echo[USART_DEVICES][USART_CHANNEL];
	struct
	{
...
void USART_WriteDecimal(USART_t* Device, const uint32_t Number, const uint8_t Base) 
{
	...
	do 
	{
		*--pBuffer = __Representation[Number_Temp % Base];
		Number_Temp /= Base;
	}while(Number_Temp != 0);
...
void USART_SetBaudrate(USART_t* Device, const uint32_t Baudrate, const uint32_t Clock, const int8_t BSCALE, const bool DoubleSpeed)
{
	...
	if(DoubleSpeed == true)
	{
		BSEL = Clock / (Baudrate << 0x03);
	}
	else
	{
		BSEL = Clock / (Baudrate << 0x04);
	}
