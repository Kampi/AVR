...
#include "Arch/AVR8/USB/Endpoint.h"
...
Bool_t Endpoint_Configure(const uint8_t Address, const Endpoint_Type_t Type, const Endpoint_Size_t Size, const uint8_t Banks)
{
	...
	for(uint8_t i = Address_Temp; i < USB_TOTAL_ENDPOINTS; i++)
	{
		...
		// Continue if the chosen endpoint is allocated already
...
void Endpoint_ClearBank(const uint8_t RequestType)
{
	...
	{
		while(!(Endpoint_IsOUTFull()));
		{
			// Cancel if device gets unattached
