...
{
	SPIM_CHIP_SELECT(&FIRST_ARG(ST7565R_SS), SECOND_ARG(ST7565R_SS));
	GPIO_Clear(&FIRST_ARG(ST7565R_REGISTER_SELECT), SECOND_ARG(ST7565R_REGISTER_SELECT));
	SPIM_TRANSMIT(&CONCAT(DISPLAY_INTERFACE), Command);
	GPIO_Set(&FIRST_ARG(ST7565R_REGISTER_SELECT), SECOND_ARG(ST7565R_REGISTER_SELECT));
	SPIM_CHIP_DESELECT(&FIRST_ARG(ST7565R_SS), SECOND_ARG(ST7565R_SS));
}
...
static void ST7565R_WriteCommandBytes(const uint8_t* Data, const uint8_t Length)
{
	...
	{
		SPIM_TRANSMIT(&CONCAT(DISPLAY_INTERFACE), *Data++);
	}
...
void Display_Init(SPIM_Config_t* Config)
{
	...
	GPIO_SetDirection(&FIRST_ARG(ST7565R_BACKLIGHT), SECOND_ARG(ST7565R_BACKLIGHT), GPIO_DIRECTION_OUT);
	GPIO_Clear(&FIRST_ARG(ST7565R_BACKLIGHT), SECOND_ARG(ST7565R_BACKLIGHT));

	// Set display reset as output in high state
	GPIO_SetDirection(&FIRST_ARG(ST7565R_RESET), SECOND_ARG(ST7565R_RESET), GPIO_DIRECTION_OUT);
	GPIO_Set(&FIRST_ARG(ST7565R_RESET), SECOND_ARG(ST7565R_RESET));

	// Set register select as output in high state
	GPIO_SetDirection(&FIRST_ARG(ST7565R_REGISTER_SELECT), SECOND_ARG(ST7565R_REGISTER_SELECT), GPIO_DIRECTION_OUT);
	GPIO_Set(&FIRST_ARG(ST7565R_REGISTER_SELECT), SECOND_ARG(ST7565R_REGISTER_SELECT));

	// Set display SS as output in high state
	GPIO_SetDirection(&FIRST_ARG(ST7565R_SS), SECOND_ARG(ST7565R_SS), GPIO_DIRECTION_OUT);
	GPIO_Set(&FIRST_ARG(ST7565R_SS), SECOND_ARG(ST7565R_SS));
...
{
	GPIO_Clear(&FIRST_ARG(ST7565R_RESET), SECOND_ARG(ST7565R_RESET));
...
void ST7565R_SwitchBacklight(const Bool_t State)
{
	...
	{
		GPIO_Set(&FIRST_ARG(ST7565R_BACKLIGHT), SECOND_ARG(ST7565R_BACKLIGHT));
	}
}
