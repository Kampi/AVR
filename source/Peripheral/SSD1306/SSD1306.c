...
{
	SPIM_CHIP_SELECT(&FIRST_ARG(SSD1306_SS), SECOND_ARG(SSD1306_SS));
	GPIO_Clear(&FIRST_ARG(SSD1306_DATA), SECOND_ARG(SSD1306_DATA));
	SPIM_TRANSMIT(&CONCAT(DISPLAY_INTERFACE), Data);
	SPIM_CHIP_DESELECT(&FIRST_ARG(SSD1306_SS), SECOND_ARG(SSD1306_SS));
}
...
void Display_Init(SPIM_Config_t* Config)
{
	...
	GPIO_SetDirection(&FIRST_ARG(SSD1306_DATA), SECOND_ARG(SSD1306_DATA), GPIO_DIRECTION_OUT);
	GPIO_Set(&FIRST_ARG(SSD1306_DATA), SECOND_ARG(SSD1306_DATA));
	
	// Set display reset as output in high state
	GPIO_SetDirection(&FIRST_ARG(SSD1306_RESET), SECOND_ARG(SSD1306_RESET), GPIO_DIRECTION_OUT);
	GPIO_Set(&FIRST_ARG(SSD1306_RESET), SECOND_ARG(SSD1306_RESET));
	
	// Set display SS as output in high state
	GPIO_SetDirection(&FIRST_ARG(SSD1306_SS), SECOND_ARG(SSD1306_SS), GPIO_DIRECTION_OUT);
	GPIO_Set(&FIRST_ARG(SSD1306_SS), SECOND_ARG(SSD1306_SS));
